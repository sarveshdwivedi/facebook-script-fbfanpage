...
module.exports = function (app, passport) {
    ...
    app.get('/profile', isLoggedIn, function (req, res) {
     ...
     app.get('/fanPageList', isLoggedIn, function (req, res) {
        console.log(req.user);
        var userRecord = req.user;
        var fanPageLists = {};
        var keywords = {};
        async.series([//you can use "async.series" as well
            function (callback) {
                //get access token
                Keyword.find({ 'fbId': userRecord.facebook.id }, { keyword: 1 }, function (err, keyword) {
                    if (err) {
                        callback(err);
                    } else {
                        console.log(keyword);
            ...
            function (callback) {
                ...
                UsersModel.findOne({ 'facebook.id': userRecord.facebook.id }, function (err, users) {
                    if (err) {
                        callback(err);
                    } else {
                        //console.log(users);
                                    ...
                                    if (err) {
                                        ...
                                        if (!pages.length > 0) {
                                            graph.extendAccessToken({
                                                ...
                                                newPage.fbId = userRecord.facebook.id;
            ...
            function (callback) {
                Page.find({ fbId: userRecord.facebook.id }, function (err, pages) {
                    if (err) {
                        callback(err);
                    } else {
            ...
            if (err) {
                res.render('fbpages.ejs', {
                    ...
                    userId: userRecord.facebook.id, // get the user out of session and pass to template
                    keywords: keywords
                });
            } else {
    ...
    app.post('/savekeyword/:id', function (req, res) {
      ...
      app.get('/getPosts/:id/:userId', function (req, res){
        console.log(req.params,"Post page");
        var postLists = {};
         async.series ([
            function (callback){
                Page.find({})
             ...
             function (callback) {
                ...
                UsersModel.findOne({ 'facebook.id': req.params.userId }, function (err, users) {
                    ...
                    if (err) {
                        ...
                        //console.log(users);
                        var tokan = users.facebook.token;
                        //var tokan = 'EAACS0WyXCp4BANmH9ditFMn
                graph.get(req.params.id + "/feed?access_token=" + tokan, function (err, posts) {
                    console.log(posts,"vdsgfgdsfgdfg");
                   Post.find({ postId: res.id }, function (err, post) {
                               //  console.log(JSON.parse(post[1].post),"posts");
                                      if (err) {
                                        console.log('err');
                                    } else {
                                       if (!posts.length > 0) {
                                            ...
                                            newPost.post = posts; // set the facebook page                   
            ...
            function (callback) {
                Post.find({ fbPageId: req.params.id }, function (err, posts) {
                    if (err) {
                        callback(err);
                    } else {
            ...
            if (err) {
                res.render('posts.ejs', {
                    ...
                    userId: req.params.userId, // get the user out of session and pass to template
                    
                });
            } else {
                
                if (postLists.posts.length > 0) {
                    console.log(postLists.posts);
                    res.render('posts.ejs', {
                        posts: postLists.posts,
                        userId: req.params.userId, // get the user out of session and pass to template
    ...
    app.get('/removeFanPageComments/:id/:userId', function (req, res) {
        ...
        async.series([//you can use "async.series" as well
            ...
            if (err) {
                ...
                res.render('successpage.ejs', {
                    pages: 'Comments has been deleted based on given keyword. Please login to your fb account and go to particular Post of fan page to check removed comments.', // get the user out of session and pass to template
                    fbId: req.params.userId
                });
            }
        });
                        ...
                        function (callback) {
                            ...
                            graph.get(pageId + "/feed?access_token=" + user_access_token, function (err, posts) {
                                if (posts.data) {
                                    var fbpost = posts.data;
                                    if (fbpost.length > 0) {
                                        fbpost.forEach(function (res,req) {
                                            Post.find({ postId: res.id }, function (err, postRecord) {
                                                if (err) {
                                                    console.log('err');
                                                } else {
